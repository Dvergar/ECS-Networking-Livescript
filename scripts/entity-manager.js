...
(function(){
  'use strict';
  var protoMessages, messages, numMessages, messageName, messageType, Entity, EntityManager, SYNC, CLIENT, SERVER, out$ = typeof exports != 'undefined' && exports || this;
  protoMessages = dcodeIO.ProtoBuf.loadProtoFile('components.proto').build();
  messages = [];
  numMessages = 0;
  for (messageName in protoMessages) {
    messageType = protoMessages[messageName];
    messages.push(messageType);
    messageType.id = numMessages;
    messageType.prototype.id = numMessages++;
